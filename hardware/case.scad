...
// ---- Handheld build ----
wall          = 2.6;      // thinner walls for a slimmer feel
tol           = 0.4;
margin        = 7;        // tight margin around the display -> compact
gap           = 10;       // gap between screen and speaker (cable slot lives here)
inner_h       = 24;       // just enough for the Pi + some cable slack
grip_r        = 12;       // big rounded corners = comfortable to hold

// ---- Footprint (kept as small as the screen + mic allow) ----
top_w   = max(disp_w, mic_dia) + 2*margin;              // ~135 mm wide
top_d   = disp_h + gap + mic_dia + 2*margin;            // ~172 mm tall
outer_w = top_w;
outer_d = top_d;
...
// Cable slot sits in the gap between them.
...
// ---------------------------------------------------------------------------
//   FRONT edge (-Y wall):  USB-C power, micro-HDMI0, micro-HDMI1, 3.5mm jack
...
// Cut an opening in the FRONT (-Y) wall at board-offset o.
module cut_front(o, w, h) {
    translate([-pi_w/2 + o, -outer_d/2 - wall/2, port_z])
        cube([w, wall*2 + 2, h], center = true);
}
// Cut an opening in the RIGHT (+X) wall at board-offset o.
module cut_right(o, w, h) {
    translate([outer_w/2 - wall/2, -pi_h/2 + o, port_z])
...
module base_tray() {
    difference() {
        union() {
            ...
            // Pi standoffs (centered on the floor).
            translate([0, 0, wall])
                for (sx = [-1, 1], sy = [-1, 1])
        ...
        for (p = front_ports) cut_front(p[0], p[1], p[2]);
